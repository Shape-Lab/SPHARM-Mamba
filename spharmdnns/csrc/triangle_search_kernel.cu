...
__global__ void triangle_search_kernel(
	int64_t *__restrict__ id,
	const float *__restrict__ vertex,
	const int64_t *__restrict__ face,
	const int64_t *__restrict__ cand,
	const int64_t ncand,
	const int64_t *__restrict__ offset,
	...
	const float eps)
{
	...
	if (tid < nvert)
	{
		...
		{
			const int64_t idx = (cand != NULL) ? cand[(offset != NULL) ? offset[tid] * ncand + i : i] : i;
			const float *n = &faceNormal[idx * 3];
			const float *q = &query[tid * 3];
			const float proj = q[0] * n[0] + q[1] * n[1] + q[2] * n[2];

			if (fabs(proj) >= fabs(inner[idx]) + eps)
			{
				const float r = inner[idx] / proj;
				if (r < 0) continue;
				const float q_proj[3] = {q[0] * r, q[1] * r, q[2] * r};

				const float *a = &vertex[face[idx * 3 + 0] * 3];
				const float *b = &vertex[face[idx * 3 + 1] * 3];
				const float *c = &vertex[face[idx * 3 + 2] * 3];
				...
				float bary[3];
				bary[0] = (qbxqc[0] * n[0] + qbxqc[1] * n[1] + qbxqc[2] * n[2]) / area[idx];
				bary[1] = (qcxqa[0] * n[0] + qcxqa[1] * n[1] + qcxqa[2] * n[2]) / area[idx];
				bary[2] = 1 - bary[0] - bary[1];

				if (bary[0] >= cond && bary[1] >= cond && bary[2] >= cond)
				{
					id[tid] = idx;
					cond = fminf(fminf(bary[0], bary[1]), bary[2]);
					if (cond >= 0) break;
				}
			}
		}
	...
	const float eps)
{
	at::TensorArg vertex_t{vertex, "vertex", 1}, face_t{face, "face", 2},
		cand_t{cand, "cand", 3}, offset_t{offset, "offset", 4},
		query_t{query, "query", 5}, faceNormal_t{faceNormal, "faceNormal", 6},
		inner_t{inner, "inner", 7}, area_t{area, "area", 8};
	at::CheckedFrom c = "triangle_search_cuda";
	at::checkAllSameGPU(c, {vertex_t, face_t, cand_t, offset_t, query_t, faceNormal_t, inner_t, area_t});
	at::checkAllContiguous(c, {vertex_t, face_t, cand_t, offset_t, query_t, faceNormal_t, inner_t, area_t});

	at::cuda::CUDAGuard device_guard(vertex.device());
	cudaStream_t stream = at::cuda::getCurrentCUDAStream();

	TORCH_CHECK(offset.numel() == 0 || offset.numel() == query.size(0));
	TORCH_CHECK(cand.numel() == 0 || cand.size(0) >= ncand);
	TORCH_CHECK(face.size(0) >= ncand);
	...
	triangle_search_kernel<<<blocks, threads, 0, stream>>>(
		...
		cand.numel() == 0 ? NULL : cand.data_ptr<int64_t>(),
		ncand,
		offset.numel() == 0 ? NULL : offset.data_ptr<int64_t>(),
